...
7. 大模型辅助生成更自然的问题表达，但题目结构和评分规则由规则引擎控制。
8. 用户回答问题。
9. 大模型理解用户自由文本回答并提取动机标签。
10. 规则引擎结合行为证据、用户选择和动机标签生成投资人格画像。
11. 规则引擎分析当前持仓与人格的适配度。
12. 大模型基于规则引擎结果生成报告解释。
13. 合规检查模块检查报告是否包含投资建议或不当表达。
14. 系统生成一份结构化报告。
...
- 规则引擎负责确定性、可审计、可回溯的部分。
- 大模型负责语义理解、自然语言交互、报告表达和合规润色。
- 大模型不得作为最终投资判断主体。
- 大模型不得直接输出买卖建议、个股推荐、目标价、收益预测或确定性投资结论。

### 36.2 规则引擎负责的部分

| 模块 | 规则引擎职责 |
|---|---|
| 交割单清洗 | 字段校验、数据类型校验、异常值处理 |
| 持仓周期构建 | 按股票从建仓到清仓切分周期 |
| 成本与盈亏计算 | 移动加权成本、实现盈亏、浮动盈亏 |
| 风险指标计算 | 仓位、回撤、波动率、换手率、集中度 |
| 行为片段识别 | 亏损补仓、追涨、杀跌、重仓等规则检测 |
| 片段评分 | 代表性分数、资金影响分、近期相关分 |
| 片段筛选 | Top 3-5 选择、多样性去重 |
| 人格评分 | 10 维人格分计算 |
| 持仓适配评分 | 风险匹配、行为匹配、集中度匹配 |
| 合规硬规则 | 禁止输出买卖建议、收益预测、目标价 |

### 36.3 大模型负责的部分

| 模块 | 大模型职责 |
|---|---|
| 字段映射辅助 | 根据字段名和样例数据建议映射关系 |
| 非主动交易识别辅助 | 理解业务摘要、备注、交易类型 |
| 行为片段总结 | 将结构化片段转成用户可理解的行为观察 |
| 个性化问题生成 | 基于片段生成事实确认、动机追问、事后复盘问题 |
| 自由文本理解 | 从用户补充回答中提取行为动机标签 |
| 风险冲突解释 | 将多个风险冲突合并成可理解的主线解释 |
| 报告撰写 | 把规则结果转化为自然语言报告 |
| 投教建议生成 | 推荐风险管理、交易纪律、复盘方法等教育方向 |
| 合规润色 | 检查并改写不当表达 |
...
| 预测价格或涨跌 | “后续大概率上涨” |
| 承诺收益或风险结果 | “这样可以避免亏损” |
| 直接计算金融指标 | 让大模型计算最大回撤、仓位、收益率 |
| 直接判定人格 | 把原始交割单丢给大模型让其直接贴标签 |
| 直接决定持仓适配度 | 大模型直接输出适配分 |
| 使用羞辱性标签 | “赌徒型”“韭菜型”等 |
...
结构化行为片段与风险指标
        ↓
大模型：总结片段、生成问题、理解自由文本
        ↓
规则引擎：人格评分、持仓适配计算、风险冲突识别
        ↓
大模型：报告解释、风险提示、投教建议
        ↓
合规检查：硬规则 + LLM 合规润色
...
规则引擎选出 3-5 个代表性片段
        ↓
Behavior Summary Agent 总结片段
        ↓
Question Generation Agent 生成问题
        ↓
Compliance Guard Agent 检查问题
        ↓
用户回答问题
        ↓
Answer Understanding Agent 理解自由文本
        ↓
规则引擎生成人格分
        ↓
规则引擎计算持仓适配度
        ↓
Report Writing Agent 生成报告
        ↓
Compliance Guard Agent 检查报告
        ↓
展示最终报告
```
...
### 38.1 Behavior Summary Agent：行为片段总结助手

#### 目标

将规则引擎识别出的结构化行为片段，转化为用户可理解、中性、非投资建议式的描述。
...
```json
{
  "behavior_observation": "你在 A 股票下跌过程中连续加仓 3 次，仓位最高达到 31%，期间最大浮亏约 22%。",
  "potential_risk": "如果这种加仓没有明确规则，可能导致单票风险逐渐扩大。",
  "tone": "neutral",
...
2. 不建议买入或卖出。
3. 不使用责备、羞辱、诊断式语言。
4. 必须基于输入证据，不得创造不存在的交易事实。
5. 输出不超过 120 字。
6. 使用“可能”“需要关注”等审慎表达。

---

### 38.2 Question Generation Agent：问题生成助手

#### 目标

基于代表性行为片段生成更自然、更个性化的问题。
...
```json
{
  "segment": {
    "behavior_type": "LOSS_AVERAGING_DOWN",
    "stock_name": "A 股票",
    "summary": "用户在 A 股票下跌过程中连续补仓 3 次，仓位从 8% 提升至 31%，最大浮亏 22%。",
    "question_themes": ["补仓动机", "仓位意识", "退出机制"]
  },
  "question_constraints": {
    "question_count": 3,
    "option_count": 5,
    "must_include": ["事实确认", "动机追问", "事后复盘"],
    "forbidden_terms": ["建议买入", "建议卖出", "目标价", "一定适合"]
  }
}
```
  ...
  "questions": [
    {
      ...
      "question_text": "系统观察到你在 A 股票下跌过程中连续补仓 3 次，仓位从 8% 提升至 31%。这个描述是否符合你当时的交易情况？",
      "options": [
        {"key": "A", "text": "完全符合"},
        {"key": "B", "text": "基本符合"},
        {"key": "C", "text": "不太符合"},
...
1. 问题必须基于真实行为片段。
2. 不问“你是不是某类人”，而问“当时为什么”。
3. 每个片段默认生成 3 道题。
4. 每题默认 5 个选项，事实确认题可为 4 个选项。
5. 选项必须能区分理性策略与行为偏差。
6. 评分映射由规则引擎绑定，不完全依赖大模型。
7. 若 LLM 输出不符合 Schema，回退到模板题库。

---

### 38.3 Answer Understanding Agent：用户回答理解助手

#### 目标

理解用户自由文本回答，提取行为动机标签，为人格评分提供辅助信息。
...
{
  "question_id": "Q001",
  "question_text": "当时你继续加仓的主要原因是什么？",
  "selected_option": "C",
  "selected_option_text": "我主要想降低持仓成本，等待反弹回本",
  "free_text": "当时觉得跌得比较多了，而且基本面没明显变差，补仓后成本低一些，反弹就能快点回本。"
}
```
...
```json
{
  "motives": [
    {
      "label": "BREAKEVEN_MENTALITY",
      "confidence": 0.86
    },
    {
      "label": "FUNDAMENTAL_REASONING",
      "confidence": 0.42
    },
    {
      "label": "AVERAGE_COST_MENTALITY",
      "confidence": 0.78
    }
  ],
  "summary": "用户同时提到基本面未变化和降低成本回本，其中回本与摊低成本动机更明显。",
  "persona_adjustments": {
    "loss_aversion": 8,
    "discipline": -5,
    "self_awareness": 2
...
```json
{
  "FUNDAMENTAL_REASONING": "基本面判断",
  "PLANNED_POSITION_BUILDING": "计划内分批建仓",
  "BREAKEVEN_MENTALITY": "回本心理",
  "AVERAGE_COST_MENTALITY": "摊低成本动机",
  "FOMO": "踏空焦虑",
  "HERDING": "从众行为",
  "PANIC_SELLING": "恐慌卖出",
  "DISCIPLINED_STOP_LOSS": "纪律性止损",
  "OVERCONFIDENCE": "过度自信",
  "REGRET_AVERSION": "后悔厌恶",
  "NO_CLEAR_PLAN": "缺乏明确计划",
  "INFORMATION_NOISE": "信息噪音驱动",
  "PROFIT_PROTECTION": "害怕利润回吐",
  "POSITION_RISK_AWARENESS": "仓位风险意识",
  "UNCERTAIN": "证据不足"
}
```
...
1. 大模型只提取动机标签和置信度。
2. 最终人格分由规则引擎根据标签进行修正。
3. 如果文本证据不足，输出 `UNCERTAIN`。
4. `persona_adjustments` 只是建议值，规则引擎需二次校验。
5. 单个维度修正值不得超过 `-10` 到 `+10`。

---

### 38.4 Report Writing Agent：报告撰写助手

#### 目标

基于规则引擎输出的结构化结果生成自然语言报告。
...
### 38.5 Compliance Guard Agent：合规检查助手

#### 目标

检查大模型生成内容中是否存在投资建议、收益预测、个股推荐、绝对化判断、侮辱性标签或不当心理诊断，并给出安全改写。

#### 检查项

| 类型 | 示例 | 处理 |
|---|---|---|
| 明确买卖建议 | 建议卖出 A 股票 | 阻断并改写 |
| 个股推荐 | 可以换成 B 股票 | 阻断 |
| 收益承诺 | 这样能降低亏损 | 改为“有助于关注风险” |
| 价格预测 | 后面可能上涨 | 阻断 |
| 绝对判断 | 你一定不适合高波动股票 | 改为审慎表达 |
| 侮辱性标签 | 赌徒型人格 | 阻断 |
| 过度诊断 | 你有严重心理问题 | 阻断 |
  ...
  "violations": [
    {
      ...
      "original_text": "建议卖出 A 股票",
      "rewrite": "建议关注 A 股票在组合中的仓位占比和波动风险"
    }
  ],
  "safe_version": "建议关注 A 股票在组合中的仓位占比和波动风险。"
}
```
...
2. 合规检查不通过时，不允许直接展示。
3. 系统可自动采用 `safe_version`。
4. 如果无法安全改写，则回退到模板化风险提示。
...
```text
你是一个投资行为分析助手。请根据结构化交易片段，生成一段中性、客观、非投资建议式的行为观察。
...
```text
你是一个投资者行为复盘问卷设计助手。请基于用户真实交易片段生成问题。

要求：
1. 每个问题必须基于片段事实。
2. 不问“你是不是某类人”。
3. 重点问“当时为什么这么做”。
4. 每题 5 个选项，事实确认题可为 4 个选项。
5. 选项必须能区分理性策略与情绪/行为偏差。
6. 不包含任何买入、卖出、加仓、减仓建议。
7. 不预测价格和收益。
8. 输出 JSON。
9. 不创造输入中不存在的交易事实。
...
```text
你是一个投资行为文本理解助手。请理解用户对交易复盘问题的回答，并提取可能的行为动机。

要求：
1. 不做投资建议。
2. 不判断用户对错。
3. 只提取动机标签和置信度。
4. 如果证据不足，请输出 UNCERTAIN。
5. 输出 JSON。
6. persona_adjustments 只是建议值，不得超过 -10 到 +10。
...
```text
你是一个投资行为复盘报告撰写助手。请根据系统计算结果生成报告文本。

严格要求：
1. 不提供具体买入、卖出、加仓、减仓建议。
2. 不预测价格和收益。
3. 不评价个股好坏。
4. 使用“可能”“需要关注”“风险在于”等审慎表达。
5. 语言中性，不责备用户。
6. 报告重点是行为复盘、风险识别和持仓适配度。
7. 必须包含免责声明。
8. 不创造输入中不存在的事实。
9. 所有结论必须可追溯到输入证据或规则评分。
...
```text
你是金融产品合规文本检查助手。请检查以下文本是否存在投资建议、收益预测、个股推荐、绝对化判断、侮辱性表达或不当心理诊断。
...
```http
POST /api/v1/llm/behavior-summary
```
...
```http
POST /api/v1/llm/questions/generate
```
...
```http
POST /api/v1/llm/answers/understand
```
...
```http
POST /api/v1/llm/compliance/check
```
...
| 字段 | 类型 | 说明 |
|---|---|---|
| llm_task_id | varchar | LLM 任务 ID |
| user_id | varchar | 用户 ID |
| task_type | varchar | SUMMARY / QUESTION / ANSWER_UNDERSTANDING / REPORT / COMPLIANCE |
| input_json | json | 输入 |
| output_json | json | 输出 |
...
### 41.2 llm_prompt_templates

| 字段 | 类型 | 说明 |
|---|---|---|
| prompt_template_id | varchar | Prompt 模板 ID |
| task_type | varchar | 任务类型 |
| version | varchar | 版本 |
| template_text | text | Prompt 内容 |
| json_schema | json | 输出 Schema |
| active | boolean | 是否启用 |
| created_at | datetime | 创建时间 |

### 41.3 llm_compliance_results

| 字段 | 类型 | 说明 |
|---|---|---|
| compliance_result_id | varchar | 检查结果 ID |
| llm_task_id | varchar | 被检查任务 ID |
| passed | boolean | 是否通过 |
| violations_json | json | 违规项 |
| safe_version | text | 安全改写 |
| created_at | datetime | 创建时间 |

### 41.4 answer_motive_labels

| 字段 | 类型 | 说明 |
|---|---|---|
| label_id | varchar | 标签记录 ID |
| answer_id | varchar | 用户回答 ID |
| motive_label | varchar | 动机标签 |
| confidence | decimal | 置信度 |
| source | varchar | LLM / RULE / MANUAL |
| created_at | datetime | 创建时间 |
...
| 失败场景 | 回退方案 |
|---|---|
| LLM 字段映射失败 | 使用规则模板或要求用户手动映射 |
| LLM 问题生成失败 | 使用固定问题模板 |
| LLM 输出 JSON 格式错误 | 重试 1 次，失败后回退模板 |
| LLM 合规检查失败 | 使用模板化安全表达 |
| LLM 报告生成失败 | 使用规则引擎模板报告 |
| LLM 回答理解置信度低 | 仅使用用户单选答案评分 |
| LLM 服务不可用 | 系统降级为纯规则版本 |
...
1. 首期仅对内部测试用户开放。
2. 记录所有 LLM 输入、输出和回退结果。
3. 人工抽检报告和问题质量。
4. 片段问题相关度低于阈值时回退模板。
5. 合规不通过率超过阈值时暂停对应 Prompt 版本。

### 44.2 质量监控指标

| 指标 | 定义 | MVP 目标 |
|---|---|---:|
| LLM 输出 JSON 合法率 | JSON Schema 通过次数 / 调用次数 | >= 98% |
| 问题合规通过率 | 合规通过问题数 / 问题总数 | >= 99% |
| 报告合规通过率 | 合规通过报告数 / 报告总数 | 100% |
| LLM 回退率 | 回退次数 / 调用次数 | <= 10% |
| 用户认为问题相关比例 | 评分 >= 4 的比例 | >= 75% |
| 人工抽检可接受率 | 抽检通过样本 / 抽检总样本 | >= 90% |
...
```text
behavior_summary_v1.0
question_generation_v1.0
answer_understanding_v1.0
report_writer_v1.0
compliance_guard_v1.0
```
...
```text
规则引擎保证确定性、可审计、可合规；
大模型负责语义理解、个性化追问、自然语言解释和合规润色。
```

该架构的目标是在不直接荐股、不做黑箱投资判断的前提下，让系统具备更强的用户理解能力、复盘能力和报告表达能力。
